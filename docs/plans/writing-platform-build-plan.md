...
- **Wave C2** (library UI + editor): ✅ landed. **Gate verdict: PROCEED** — redaction attack-resistant at response-text level, editor byte path verified sound. Post-gate hardening applied: handleSave enforces the read-only contract itself, save refuses ill-formed strings (lone surrogates would silently become U+FFFD), e2e mocks use the server's real enveloped list shapes.
- **Wave D1** (compiler): ✅ landed. **Gate verdict was HOLD** — resolved same-session: (1) the demonstrated denylist-straddle emission is fixed (scan runs over the joined outbound buffer; regression test added); (2) the reviewed pin table ships in `pins.ts` (style-lock v3 hash + all 8 CORE hashes from howl-to-heaven@e7aa5c6) and `compilePanelPrompt` refuses `pin-unknown`/`pin-mismatch`/`style-lock-unpinned` against it — a caller can no longer bless its own bytes; the env-gated oracle recomputes the table from the real repo so canon edits force a pins.ts update in the same commit; (3) invariant scope is now explicit, below.

### Invariant ownership (I1–I13 re-scope, corrected after reviewer gates 3 and 4)

**Compiler, enforced in code:** I2 required-substring over *all six* verbatim fields (style lock, every CORE, every NEGATIVE, set, color rules, shot); I3 via style-lock hash + reviewed pins; I4 franchise denylist over the whole buffer **and** its whitespace-stripped form (CJK terms and line-wrapped terms have no `\s+` tolerance — gate 4 proved the raw-only scan missed them); I9 pin table + oracle (see the CI caveat below); I12 residue — `aspect-unknown` plus the aspect directive now emitted into DIRECTION and returned on the result (`大格` tiebreak stays resolution-phase); guardrail (a) softening denylist over DIRECTION and SHOT (CONTINUITY/NEGATIVE excluded by design: the real NEGATIVE text says "no adult proportions"); stage gates `stage-text-only` (content-tested, not identity-tested) and `stage-anchor-missing`.

**Compiler, NOT enforced — caller's responsibility today:** I1's mandatory block literals (the spec's fixed DIRECTION/NEGATIVE boilerplate) are pass-through; the compiler guarantees block *order and count*, not that the caller supplied the standard phrases.

**Server seams (Stage-2, MUST land before generation wiring):** I5/I6 attachment ownership + approval (`attach_own_anchor`), I11 TRUE-FORM replace-path for ep0_p39_g1, I13 shot_approved + hua_sha256 freshness — all resolution-phase, live where panel identity exists.

**PreToolUse hooks at the agent boundary (architecture §3.3):** I7 no-seed, I8 write-ahead manifest row, I10 retry byte-identity. *Nothing in the compiler checks for a manifest row* — the Wave D scope line that promised a "write-ahead check hook shape" was not delivered and is re-scoped here.

**I9 CI caveat (honest statement):** canon lives in a separate repository, so "fails the build until the pin is updated in the same commit" is not expressible in this repo's CI — there is no checkout to hash against, and the pins oracle is `skipIf(!HTH_REPO_PATH)`. The operational gate is `HTH_REPO_PATH=… npm run verify:canon`, which must be run after any canon edit and before any generation run. This is a process guarantee, not a mechanical one; see open question Q6.

**Open author ruling**: the denylist matches 死神 only adjacent to "Bleach" (it is an ordinary word — death god — that appears legitimately in supernatural canon text). Spec I4 lists it unqualified. Flag for the author; narrow match kept until ruled otherwise.
