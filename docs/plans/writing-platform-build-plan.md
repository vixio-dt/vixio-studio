...
### Invariant ownership (I1–I13 re-scope, per reviewer gate 3)

Compiler (landed): I1/I2 structural, I3 via style-lock hash + pins, I4 whole-buffer denylist, I9 pins module + oracle, I12 residue (`aspect-unknown`; `大格` tiebreak is resolution-phase), stage gates (text-only, anchor-missing). Server seams (Stage-2, MUST land before generation wiring): I5/I6 attachment ownership + approval (`attach_own_anchor`), I11 TRUE-FORM replace-path for ep0_p39_g1, I13 shot_approved + hua_sha256 freshness — all resolution-phase, live where panel identity exists. PreToolUse hooks at the agent boundary (architecture §3.3): I7 no-seed, I8 write-ahead manifest row, I10 retry byte-identity.
